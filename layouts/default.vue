...
<div class="toggle-btn" @click="showing = !showing, menu()">
          <span class="one"></span>
          <span class="two"></span>
...
// tl3.paused();
 tl3
  ...
  .staggerFromTo(".button-background", 1, {scale: "0"}, {scale: "100", autoAlpha: 1, ease: Power1.easeInOut}, 1)
  .add('show-menu')
    //  .staggerFromTo(".toggle-btn", .5, {rotation:0}, {rotation:360, ease:Linear.easeNonet}, .5)
  .staggerFromTo(".bottom", .10, {opacity:1}, {opacity:0, ease:Bounce.easeNonet}, .10)
  .staggerFromTo(".one", .10, {rotation:0, y: 0}, {rotation:45, y: 10, ease:Bounce.easeNonet}, .10)
  .staggerFromTo(".two", .10, {rotation:0}, {rotation:-45, ease:Bounce.easeNonet}, .10)
  
  .staggerFromTo("body", .15, {overflow:"initial"}, {overflow:"hidden"}, .75)

  .staggerFromTo(".menu", .15, {autoAlpha:0}, {display: "initial",autoAlpha:1}, "show-menu", .75)
  .addPause();
