<template>
 ...
 <div v-for="image in images" :class="`img-galleries slide-`+image.id" :key="image.id" :style="{ 'background-image': 'url(' + image.location + ')' }" width="1" height="1">
...
for(var i = slides.length; i > 1 ; i--){
  ...
  tl
  .add('intro')
  .staggerFromTo(".slide-"+i, 1, {autoAlpha:1}, {opacity:0, ease: Power2.easeInOut}, .25)
  .fromTo(".slide-"+(i-1), .25, {opacity:0 }, {autoAlpha: 1,ease: Power0.easeInOut},'intro', .25)
  .addPause();
}
...
for(var j = slides.length; j > 1 ; j--){
  ...
  tl2
  .add('intro-text')
  .staggerFromTo(".text-"+j, .5, {autoAlpha:1}, {opacity:0, ease: Power2.easeInOut}, .5)
  .fromTo(".text-"+(j-1), .5, {opacity:0}, {autoAlpha: 1,ease: Power0.easeInOut}, 'intro-text', .5)
